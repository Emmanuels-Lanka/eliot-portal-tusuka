...
const AddSewingMachineForm = ({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      unitId: initialData?.unitId || undefined, // Use undefined instead of empty string
      machineType: initialData?.machineType || undefined,
  ...
  // Filter out any invalid data to prevent empty SelectItem values
  const validUnits = units?.filter(unit => unit.id && unit.id.trim() !== '' && unit.name) || [];
  const validMachineTypes = machineTypes?.filter(type => type.code && type.code.trim() !== '' && type.name) || [];
  const validBrands = MACHINE_BRANDS?.filter(brand => brand.name && brand.name.trim() !== '') || [];
              ...
              render={({ field }) => (
                <FormItem className="my-4 ">
                  ...
                  <Select
                    ...
                    value={field.value || ""} // Use controlled value instead of defaultValue
                  >
                    <FormControl>
                      <SelectTrigger>
